...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btn_confirm_suppression'])) {
    ...
    $stmt = $connexion->prepare("DELETE FROM specialite WHERE code_spec = ?");
    ...
    try{
        if ($stmt->execute()) {
        header("Location: ../index.php?cat=specialite&page=affichage&res=del_ok&id=".$id_suppression);
        exit();
        }
        else {
         header("Location: ../index.php?cat=specialite&page=affichage&res=error_fk&id=".$id_suppression);
          }
    }
